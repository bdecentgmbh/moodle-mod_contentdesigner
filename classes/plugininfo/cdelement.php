...
class cdelement extends \core\plugininfo\base {
    ...
    public static function update_plugins_config(string $pluginname, $newpluginname, $areafiles = []) {
        ...
        $DB->set_field_select('config_plugins', 'plugin', $newpluginname, 'plugin=:pluginname AND name NOT IN (
            SELECT cp.name FROM {config_plugins} cp where cp.plugin=:newpluginname)', ['pluginname' => $pluginname, 'newpluginname' => $newpluginname]);
        ...
        if (class_exists('\core\output\progress_trace\progress_trace_buffer')) {
            ...
            $progress = new \core\output\progress_trace\progress_trace_buffer(new \core\output\progress_trace\text_progress_trace(), false);    
        } else {
