...
class provider implements
    ...
    public static function export_element_user_data(array $contentdesignerids, \stdclass $user) {
        ...
        $sql = "SELECT ec.*, ecc.userid AS userid, ecc.completion AS completion, ecc.timecreated AS timecompleted
            FROM {element_chapter} ec
            INNER JOIN {element_chapter_completion} ecc ON ecc.instance = ec.id AND ecc.userid = :userid
    ...
    public static function get_users_in_context(userlist $userlist) {
        ...
        FROM {element_chapter} ec
        JOIN {element_chapter_completion} ecc ON ecc.instance = ec.id
    ...
    public static function delete_data_for_users(approved_userlist $userlist) {
        ...
        $list = $DB->get_records('element_chapter', ['contentdesignerid' => $contentdesigner->id]);
        $ids = array_column($list, 'id');
        list($insql, $inparams) = $DB->get_in_or_equal($ids, SQL_PARAMS_NAMED, 'ch');
        $DB->delete_records_select('element_chapter_completion', "userid {$userinsql} AND instance $insql",
    ...
    public static function delete_data_for_user(approved_contextlist $contextlist) {
        ...
        foreach ($contextlist->get_contexts() as $context) {
            ...
            $list = $DB->get_records('element_chapter', ['contentdesignerid' => $instanceid]);
            $ids = array_column($list, 'id');
            list($insql, $inparams) = $DB->get_in_or_equal($ids, SQL_PARAMS_NAMED, 'ch');
            $DB->delete_records_select('element_chapter_completion', "userid=:userid AND instance $insql",
    ...
    public static function delete_data_for_all_users_in_context(\context $context) {
        ...
        $list = $DB->get_records('element_chapter', ['contentdesignerid' => $cm->instance]);
        $ids = array_column($list, 'id');
        list($insql, $inparams) = $DB->get_in_or_equal($ids, SQL_PARAMS_NAMED, 'ch');
        $DB->delete_records_select('element_chapter_completion', "instance $insql", $inparams);
